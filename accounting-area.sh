...
if ! [ -f $RRD_ROOT/qacct_avail.rrd ] ; then
       rrdtool create $RRD_ROOT/qacct_avail.rrd -s 180 $creabbdd RRA:AVERAGE:0.5:1:576
fi
...
rrdtool graph $WEB_ROOT/img/month.png -a PNG -s -1month -t "HPC Usage (Monthly)" -h 200 -w 600 -v "Load" COMMENT:" \\l" $datagrups COMMENT:" Last update\: $DATE" > /dev/null

rrdtool graph $WEB_ROOT/img/year.png -a PNG -s -1year -t "HPC Usage (Yearly)" -h 200 -w 600 -v "Load" COMMENT:" \\l" $datagrups  COMMENT:" Last update\: $DATE" > /dev/null
